...
import Logo from '../../assets/logo/eduforge.png'

const AuthContainer = ({ children, title, subtitle }) => {
	return (
		<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="w-full max-w-lg h-full max-h-175 overflow-y-auto bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-2xl scollbar-none">
			<div className="text-center mb-8">
				<motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: 'spring', stiffness: 200 }} className="flex justify-center mb-4">
					<div class="p-[2px] relative size-15 rounded-full flex items-center justify-center">
						<img src={Logo} alt="EduForge Logo" />
					</div>
				</motion.div>
